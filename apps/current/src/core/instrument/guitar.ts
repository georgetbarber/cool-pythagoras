...
export function positionMovement(
  from: readonly Pick<FretPosition, "midi" | "fret">[],
  to: readonly Pick<FretPosition, "midi" | "fret">[]
): number {
  if (!from.length || !to.length) return 0;
  const source = [...from].sort((a, b) => a.midi - b.midi);
  const target = [...to].sort((a, b) => a.midi - b.midi);
  const voiceDistance = target.reduce((total, position) => {
    const nearest = Math.min(...source.map((item) => Math.abs(item.midi - position.midi)));
    return total + nearest;
  }, 0);
  const sourceCentre = source.reduce((sum, position) => sum + position.fret, 0) / source.length;
  const targetCentre = target.reduce((sum, position) => sum + position.fret, 0) / target.length;
  return voiceDistance + Math.abs(sourceCentre - targetCentre);
}

export function nearestShape(chord: Chord, from: readonly FretPosition[]): GuitarShape | null {
  const shapes = generateShapes(chord, Math.min(4, chord.tones.length), 24);
  return [...shapes].sort((a, b) =>
    positionMovement(from, a.positions) - positionMovement(from, b.positions) ||
    a.span - b.span ||
