        ...
        <section className="unit-stage">
          ...
          <div className="unit-grid">
            {CURRICULUM.filter((unit) => unit.stage === active.stage).map((unit) => {
              ...
              const blocker = locked ? CURRICULUM.find((candidate) => candidate.order < unit.order && unitProgress(state, candidate.id) < 100) : undefined;
              return <article className={`unit-card card ${unit.id === active.id ? "is-active" : ""} ${locked ? "is-locked" : ""}`} key={unit.id}><div className="unit-number"><span>{unit.order}</span><i style={{ "--progress": `${progress}%` } as React.CSSProperties} /></div><h3>{unit.title}</h3><p>{unit.outcome}</p><div className="competency-tags">{unit.competencyIds.map((id) => <span key={id}>{id.split(":")[0]}</span>)}</div><button disabled={locked} className="secondary-action" title={blocker ? `Finish “${blocker.title}” first` : undefined} onClick={() => dispatch({ type: "openUnit", unitId: unit.id })}>{locked ? (blocker ? `Unlocks after Unit ${blocker.order}` : "Locked for now") : progress ? "Continue unit" : "Open unit"}</button>{locked && blocker && <small className="lock-note">Finish “{blocker.title}” first</small>}</article>;
            })}
          </div>
