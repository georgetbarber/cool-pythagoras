...
import { analyzeRelationship } from "../core/music/theory";
      ...
      <div className="explore-layout">
        <section className="panel relationship-map">
          ...
          <div className="relationship-spokes">
            <article><small>Scale</small><strong>{relationship.scaleDegree ? `D${relationship.scaleDegree.degreeLabel}` : "outside"}</strong><span>{relationship.scaleDegree?.intervalName}</span></article>
            <article><small>Chord</small><strong>{relationship.chordTone ? `C${relationship.chordTone.intervalLabel}` : "non-chord"}</strong><span>{model.activeChord.symbol}</span></article>
          ...
          {containing.map((chord) => {
            ...
            return (
              <button onClick={() => dispatch({ type: "selectChord", degree: chord.degree })} key={chord.id}>
                ...
                <small>C{role?.intervalLabel} · {chord.functionLabel}</small>
              </button>
            );
          })}
        </section>
