...
const PRACTICE_MODES: Array<{ id: string; title: string; kinds: ActivityKind[]; purpose: string }> = [
  { id: "sound", title: "Sound and technique", kinds: ["technique", "play-reveal"], purpose: "Control attack, release, muting, touch and unnecessary tension." },
...
export function Practice() {
  ...
  const next = available.find((activity) => !state.completedActivityIds.includes(activity.id)) ?? available[0];
  ...
  return (
    <div className="page-stack">
      <header className="page-header compact"><div><span className="eyebrow">Targeted practice</span><h1>Train the weakest link without losing the music.</h1><p>Practice recommendations distinguish independent work from hints and reveals. Accuracy is only one kind of evidence; production, transfer and creation matter too.</p></div></header>
      ...
      <section className="evidence-panel card">
        ...
        {mastery.length ? <div className="mastery-grid">{mastery.slice(-12).map((item) => <article key={item.competencyId}><strong>{item.competencyId.split(":")[0]}</strong><span className={`mastery-state state-${item.state}`}>{item.state.replace("-", " ")}</span><small>{item.successfulDays} independent days · {item.contextCount} contexts</small>{item.assistedAttempts > 0 && <small>{item.assistedAttempts} assisted attempt{item.assistedAttempts === 1 ? "" : "s"} kept separate</small>}</article>)}</div> : <p>No evidence yet. Begin with today’s session; the system will build a picture from actual attempts rather than clicks.</p>}
      </section>
    </div>
  );
}
