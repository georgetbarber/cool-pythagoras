...
describe("V7 guitar relationship model", () => {
  ...
  it("selects a playable next shape using physical proximity", () => {
    const chords = buildChords(createContext("C", "major"));
    const source = generateShapes(chords[0])[0];
    const nearest = nearestShape(chords[3], source.positions);
    expect(nearest).not.toBeNull();
    const alternatives = generateShapes(chords[3], 3, 24);
    expect(positionMovement(source.positions, nearest!.positions)).toBe(
      Math.min(...alternatives.map((shape) => positionMovement(source.positions, shape.positions)))
    );
  });
});
