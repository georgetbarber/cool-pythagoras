...
export function buildScale(context: TonalContext): ScaleTone[] {
  ...
  return mode.intervals.map((interval, index) => ({
    ...
    name: spellName(
      LETTERS[(tonicLetter + index) % LETTERS.length],
...
function functionFor(mode: ModeId, degree: number): {
  ...
  if (mode === "major" && [2, 4].includes(degree)) {
    return { family: "predominant", label: "Preparation", explanation: "Moves away from home and prepares dominant tension." };
  }
  if (mode === "major" && [5, 7].includes(degree)) {
    return { family: "dominant", label: "Directed tension", explanation: "Contains tones that strongly point back toward tonic." };
  }
  if ([3, 6].includes(degree)) {
    return { family: "tonic-colour", label: "Tonic colour", explanation: "Shares tones with tonic and extends its stable region." };
  }
  return { family: "modal", label: "Modal colour", explanation: `Expresses the characteristic sound of ${MODES[mode].name}.` };
}
