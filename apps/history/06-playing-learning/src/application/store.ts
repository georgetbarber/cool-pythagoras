...
const STORAGE_KEY = "guitar-academy-v6";
...
const ROUTES: RouteId[] = [
  ...
  "progressions", "ear", "practice", "lab", "play", "profile"
];
...
function loadState(): AppState {
  try {
    ...
    if (![2, STORAGE_VERSION].includes(parsed.version ?? 0) || !parsed.state) return DEFAULT_STATE;
    ...
    return {
      ...
      learning: {
        ...EMPTY_RECORD,
        ...state.learning,
        skills: Object.fromEntries(Object.entries(state.learning?.skills ?? {}).map(([skill, evidence]) => [
          skill,
          {
            ...evidence,
            dueAt: evidence.dueAt ?? evidence.lastSeen,
            intervalDays: evidence.intervalDays ?? 0,
            ease: evidence.ease ?? 2.2,
...
export function appReducer(state: AppState, action: AppAction): AppState {
  switch (action.type) {
    ...
    case "selectChord":
      return { ...state, selectedChordDegree: action.degree, selectedShapeIndex: 0 };
    case "selectInterval":
    ...
    case "recordExercise":
      return {
        ...
        learning: recordEvidence(
          state.learning,
          action.skill,
          `${state.context.tonicName}-${state.context.mode}`,
          action.correct
        )
      };
  }
